...
#install.packages(c("GGally","xtable")) #if required
library(tidyverse)
library(GGally)
library(xtable)
...
INTRODUCTION STUFF FOR LOGISTIC MODEL....................................................
...
Heart cathetisation is sometimes performed on children with congenital heart defects. We want to find if it is possible to predict the required length of catheter. The height, weight and catheter length was recorded for 12 individuals.
...
\item{\textit{Linearity}}
\begin{itemize}
\item In the residuals vs fitted plot, the data follows a reasonably linear trend with an almost even spread of points above and below the horizontal line. This lack of linearity can be explained by the small data set, hence the data is approximately linear and the assumptions holds.
\end{itemize}
\item{\textit{Homoscedasticity}}
\begin{itemize}
\item In the residuals vs fitted plot, there is considerable fanning in the points, particularly for the smaller fitted values. Similarly, in the scale location plot the data has some curvature present in it further implying a non-constant variance. However, due to the small data set, this is insufficient evidence to invalidate the assumption of homoscedasticity. 
...
We created confidence intervals for further investigation of how the predictor variable and the response variable are interacting. They allow us to predict which variables will be statistically significant in our model.
...
There are 3 key assumptions to check in a logistic model of the data.

Firstly, logistic regression requies the dependent varible be a binary varible, which is the case in this model.

Secondly, indepence between the obersvations in teh data is required. As each oberservation is a different individual and breast cancer is not contagious, the data can be assumed to be independent.

Thirdly, and finally, an assumption of logistic regression is the linearlity between the log odds of the dependent varibles and the independent varibles. This can be seen to be true below.

### Code that find log odds
```{r}

# I'm using the above predict data, if this is used in the final approach, this section will need to be moved lower.
...
print.default("PLEASE DO SOME LOG ODDS OR THIS WILL LOOK VERY OUT OF PLACE")
plot(mammo.predict$Age, log(mammo.predict$predict.prob/(1-mammo.predict$predict.prob)))
# Thats cooked
```
...
pred1 <- predict(mod.step, newdata = data.frame(Age = age,
                                                Margin = c("1", "2", "3", "4", "5"),
                                                Shape = "1 and 2"), type = "response")
pred3 <- predict(mod.step, newdata = data.frame(Age = age,
                                                Margin = c("1", "2", "3", "4", "5"),
                                                Shape = "3"), type = "response")
pred4 <- predict(mod.step, newdata = data.frame(Age = age,
                                                Margin = c("1", "2", "3", "4", "5"),
                                                Shape = "4"), type = "response")

pred <- cbind(pred1, pred3, pred4)

diag.table <- data.frame(round(pred, 2), row.names = c("Circumscribed", "Microlobulated",
